...
string NewickLex::ToNewickString(Node* root, bool addBranchLengthToLabel)
{
    string str;
    WriteNodeChildren(str, root, addBranchLengthToLabel);
    str += ";";
    return str;
}
...
void NewickLex::WriteNodeChildren(string &str, Node* curNode, bool addBranchLengthToLabel)
{
    if (curNode->IsLeaf())
    {
        str += curNode->GetLabel();

        if (addBranchLengthToLabel)
    ...
    else
    {
        ...
        for (int i = 0; i < curNode->GetNbChildren(); i++)
        {
            ...
            WriteNodeChildren(str, child, addBranchLengthToLabel);
        }
        str += ")" + curNode->GetLabel();
